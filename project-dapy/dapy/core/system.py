...
class Synchrony(ABC):
    min_delay: timedelta = field(default=timedelta(milliseconds=1))
    def __post_init__(self):
        if self.min_delay <= timedelta(microseconds=0):
...
class Synchronous(Synchrony):
    ...
    def __post_init__(self):
        ...
        if self.max_delay < self.min_delay:
            raise ValueError("Maximum delay must be gat least as great as the minimum delay.")
    def get_type() -> SynchronyType:
...
@dataclass(frozen=True)
class Asynchronous(Synchrony):
    """
    Class to represent an asynchronous system.
    """
    base_delay: timedelta = field(default=timedelta(seconds=10_000))
...
class PartiallySynchronous(Synchrony):
    """
    ...
    def arrival_time_for(self, sent_at: time) -> time:
        if sent_at < self.gst:
            ...
            match random.choice(["short", "long", "long", "long", "lost", "lost"]):
                case "short":
            ...
            return sent_at + self.max_delay * random.uniform(0, 1)

@dataclass(frozen=True)
class StochasticExponential(Synchrony):
    """
    ...
    def __post_init__(self):
        super().__post_init__()
        if self.delta_t <= timedelta(microseconds=0):
...
class System:
    ...
    synchrony: Synchrony = field(default_factory=Asynchronous)
